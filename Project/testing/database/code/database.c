...
/*int main(){
     ...
     create_database();
     user u1 = {1, "Jim", "qewy", 1, 0, 'A'};
     user u2 = {2, "Jom", "qwy", 2, 0, 'B'};
     user u3 = {3, "Jam", "ewy", 2, 1, 'B'};
     user u4 = {4, "Jem", "yeq", 4, 0, 'B'};
    ...
    printf("TestCase 1: Insert account at first place of list:\n");
    insert_account(&a1); // TestPath [1,2,6,3,4,7,9,10]
    printf("TestCase 2: Insert account at second place of list:\n");
    insert_account(&a2); //TestPath [1,2,6,3,5,6,3,4,7,9,10]
    printf("TestCase 3: Account limit set to 0:\n");
    //Set AccountLimit to 0 //TestPath [1,2,6,7,8,10]
    printf("TestCase 4: No insertion as Account List is full:\nl");
    //Set AccountLimit to 1 //TestPath [1,2,6,3,5,6,7,8,10]
    printf(".............\n");
    ...
    printf("get_account_index.....\n");
    printf("Test Case 1: Get Account index of first element :%d\n",get_account_index(a1.account_id));//[1,2,6,3,4,8]
    printf("TestCase 2: Account limit set to 0\n");
    //Set AccountLimit to 0 //TestPath [1,2,6,7,8]	
    printf("Test Case 3: Get Account index of Second element :%d\n",get_account_index(a2.account_id));// Testpath [1,2,6,3,5,6,3,4,8]
    printf("Test Case 4: Get Account index of element not present in list :%d\n",get_account_index(a3.account_id));//TestPath [1,2,6,3,5,6,3,5,6,7,8]
    printf(".............\n");

    //LockAccount()

    printf("LockAccount.......\n");
    printf("TestCase 1: Unlocked:\n");
    lock_account(get_account_index(a1.account_id), -1); // TestPath [1,2,4,6,7,8,9,11]
    printf("TestCase 2: Readlocked:\n");
    lock_account(get_account_index(a2.account_id), 1); // TestPath [1,2,4,5,8,10,11]
    lock_account(get_account_index(a2.account_id), -1);
    printf("TestCase 3: Write locked:\n");
    lock_account(get_account_index(a1.account_id), 0); // TestPath [1,2,3,8,10,11]
    lock_account(get_account_index(a1.account_id), -1);
    printf("TestCase 4: Not the defined type of lock:\n");
    lock_account(get_account_index(a2.account_id), 3); // TestPath [1,2,4,6,8,10,11]
    lock_account(get_account_index(a2.account_id), -1);
    printf("..............\n");
    ...
    printf("Transaction......\n");
    transaction(a1.account_id, -10000); // TestPath [1,2,4,5,8]
    printf("Test Case 1: Get Balance after testcase 1 execution:%d\n",get_balance(a1.account_id)); 
    transaction(a2.account_id , -30000); // TestPath [1,2,4,6,8]
    printf("Test Case 2: Get Balance after testcase 2 execution :%d\n",get_balance(a2.account_id)) ;
    transaction(a3.account_id, 20000); // TestPath [1,2,3,8]
    printf("Test Case 3: Get Balance after testcase 3 execution %d\n",get_balance(a3.account_id)); 
    printf("..............\n");

    //CheckPassword()

    insert_user(&u3);
    int b1 = check_password(u1.user_id, "qewy"); // TestPath [1,2,4,5,7,8,12]  
    int b2 = check_password(u2.user_id, "qwy"); //Testpath [1,2,4,5,7,9,12] 
    int b3 = check_password(u3.user_id, "ewy"); //TestPath [1,2,4,5,6,12]
    int b4 = check_password(u2.user_id, "tyy"); //TestPath [1,2,4,10,12]
    int b5 = check_password(u4.user_id, "xyz"); // TestPath [1,2,3,12] 
    ...
    //deleteUser()
    printf("DeleteUser.....\n");
    printf("Test Case 1: Delete user with no of accounts 1 :%d\n", delete_user(u2.user_id)); // TestPath [1,2,4,5,7,8]
    printf("Test Case 2: Delete user with no of accounts 2 :%d\n", delete_user(u1.user_id));  // TestPath [1,2,4,6,7,8]
    printf("Test Case 3: Delete user not present in list :%d\n", delete_user(u4.user_id)); // TestPath [1,2,3,8]
    printf("..............\n");
